...
public class Predict {
    ...
    public List<Project> requirementPredict(ArrayList<String> resources) {
    	...
    	for (int i = 0; i < allProjects.size(); i++) {
			...
			for (int j2 = 0; j2 < allProjects.get(i).getResourceList().size() ; j2++) {

			if (allProjects.get(i).getResourceList().get(j2).getName().equals((resources.get(j)))) {
    ...
    public List<Project> resourcePredict(ArrayList<String> technologies 
    		,Domain humanResources, Domain users , Domain modules) {
    	result = new ArrayList<>();
    	List<Project> allProjects = Main.dbManager.getProjects();
    	if(allProjects != null){
    	for (int i = 0; i < allProjects.size(); i++) {
